...
const apiBase = apiUrl.apiUrl + "/contractpaymentsetting"; // your backend route prefix
const apiBases = apiUrl.apiUrl + "/inspectionevaluation";
...
export async function getAllPaymentSettingbycontractid(contractid) {
  try {
    const response = await http.post(`${apiBase}/settings/contract`,{contractid});
    return response.data;
  } catch (ex) {
    if (ex.response && ex.response.status === 503) {
      handleServiceUnavailable();
...
export async function getAllPaymentSettingbyserviceorder(service_order_id) {
  try {
    const response = await http.post(`${apiBase}/settings/serviceorder`,{service_order_id});
    return response.data;
  } catch (ex) {
    if (ex.response && ex.response.status === 503) {
      handleServiceUnavailable();
...
export async function deletePaymentSetting(id) {
  try {
    ...
    console.error("Failed to delete payment setting......." , { autoClose: 5000 });
    return false;
  }
}
...
// Get installments by serviceorder
export async function getInstallmentsByservice_order_id(service_order_id) {
  try {
    const response = await http.post(`${apiBase}/installment/serviceorder`, {
      service_order_id,
    });
    return response.data;
  } catch (ex) {
...
// Get installments by contract
export async function getInstallmentsBycontractid(contractid) {
  try {
    const response = await http.post(`${apiBase}/installment/contract`, {
      contractid,
    });
    return response.data;
  } catch (ex) {
...
// Get installments by contract
export async function getInstallmentsByserviceorderid(service_order_id) {
  try {
    const response = await http.post(`${apiBase}/installment/serviceorder`, {
      service_order_id,
    });
...
export async function addInstallment(installmentData) {
  try {
    const payload = {
...
export default {
  ...
  getInstallmentsByserviceorderid,
  addInstallment,
  getInstallmentsBycontractid,
   getInstallmentsByservice_order_id,
  deleteInstallment,
};
