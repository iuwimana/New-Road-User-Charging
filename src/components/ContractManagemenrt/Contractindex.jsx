...
const Mailbox = () => {
  ...
  if (node.type === "fiscalYear") {
    ...
    return response.data.map((item) => ({
      ...
      name: (
        <div className="border-l-4 border-indigo-500 bg-indigo-50 p-2 rounded-md shadow-sm hover:bg-indigo-100 transition-all">
          <strong className="text-indigo-700 text-lg">Contract Type:</strong>{" "}
          <span className="font-medium text-gray-800">{item.contracttypename}</span>
        </div>
      ),
      type: "contractType",
    }));
  ...
  // --------------------------------------------------------------------------
    ...
    return response.data
      ...
      .map((item) => ({
        id: item.contractid,
        name: (
          <div className="border-l-4 border-amber-500 bg-amber-50 p-2 rounded-md shadow-sm hover:bg-amber-100 transition-all">
            <strong className="text-amber-700 text-lg">Service Order:</strong>{" "}
            <span className="font-semibold text-gray-900">{item.contractrefnumber}</span>
            <span className="text-sm text-gray-600 ml-1">({item.contractmode})</span>
          </div>
        ),
        type: "serviceorder",
      }));
  ...
  // Future extension: handle contractMode, projectMode, etc.
  // else if (node.type === "contractMode") { ... }
            ...
            const fetchData = async () => {
                try {
                    ...
                    if (!business) {
                        ...
                        setBusiness(business);
                        // ✅ make sure aggreg is safe
                        if (!aggreg || aggreg.length === 0) {
                            setAggreg({}); // empty object instead of undefined
                        } else {
                            setAggreg(aggreg[0]);
                        }
                    }
                    ...
                    <Card className="shadow border-0">
                        <CardHeader className="card-header bg-transparent">
                            <div className="text-center">
                                <h1>Contract Settingst - Road Contract</h1>
                            </div>
                        </CardHeader>
                        <CardBody>
                            ...
                            <div className="mb-4">
                                <div className="d-flex justify-content-between mb-3 ">
                                    ...
                                    <div className="card text-center shadow-sm  mx-2 " style={{ flex: 1 }}>
                                        <div className="card-body">
                                            <h6 className="card-title">contract Amount for Execution</h6>
                                            <p className="card-text">{aggreg.total_budget || 0}%</p>
                                        </div>
                                    </div>
                                    <div className="card text-center shadow-sm  mx-2 " style={{ flex: 1 }}>
                                        <div className="card-body">
                                            <h6 className="card-title">contract Amount for supervision</h6>
