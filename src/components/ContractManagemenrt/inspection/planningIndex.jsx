...
import { ClipboardList, Menu, ClipboardCheck, CheckSquare, SearchCheck, FileCheck, ListChecks } from 'lucide-react';
//---------------------------------------
...
//------------
import UpdateProjectModal from './InspectionRejectedModal';
...
import { getInspectionEvaluations, getEvaluationsByPlan,getEvaluationsByPlanserviceorder, deleteEvaluation,approvEvaluation,approvEvaluationserviceorder } from '../../../services/ContractManagement/ContractSetting/inspectionEvaluationService';
//-----------------------------------------
import AddInspectionEvaluationModal from './AddInspectionEvaluationModal';
import UpdateInspectionEvaluationModal from './UpdateInspectionEvaluationModal';
import EvaluationDetailModal from './EvaluationDetailModal';

//-----------------------------------------
import AddInspectionEvaluationServiceOrderModal from './AddInspectionEvaluationServiceorderModal';
import UpdateInspectionEvaluationServiceOrderModal from './UpdateInspectionEvaluationServiceOrderModal';
import EvaluationDetailServiceOrderModal from './EvaluationDetailserviceorderModal';
...
import { getInstallmentsBycontractid,getInstallmentsByservice_order_id } from '../../../services/ContractManagement/ContractSetting/contractpaymentService';
...
import { getInspectionPlanPDF } from '../../../services/ContractManagement/ContractSetting/inspectionplanDetailServices';
...
const Mailbox = () => {
    ...
    const TreeNode = ({ node, level, fetchChildren }) => {
        ...
        const handledisplay = () => {
            ...
            if (node.type == 'fiscalYear') {
                ...
                setcontractType(node.id);
                setcontractid(node.id);
                setcancreateserviceorder(node.cancreateserviceorder);
            } else if (node.type == 'inspection') {
                setcontractType(node.id);
                setserviceorderid(node.id);
                setcancreateserviceorder(node.cancreateserviceorder);
            } else if (node.type === 'inspectionplan') {
                setcontractType(node.id);
                setcontractid(node.ids);
                setcancreateserviceorder(node.cancreateserviceorder);
            }else if(node.type=='serviceorderinspectionplan'){
                setcontractType(node.id);
                setserviceorderid(node.ids);
                setcancreateserviceorder(node.cancreateserviceorder);
        ...
        if (node.type === 'fiscalYear') {
            ...
            return (
                response.data
                    ...
                    .map((item) => ({
                        cancreateserviceorder: item.cancreateserviceorder,
                        ids: item.contractid,
                        id: item.contractid,
            ...
            try {
                // 1. Fata service orders
                const response = await ServiceOrderdata.getServiceOrders(node.ids);

                // niba response iri object yonyine (atari {data: []})
                const serviceOrdersRaw = response.data || response;

                const serviceOrders = (serviceOrdersRaw || []).map((item) => ({
                    cancreateserviceorder: item.cancreateserviceorder || false,
                ...
                // 2. Niba habonetse service orders
                if (serviceOrders.length > 0) {
                    return serviceOrders;
                }

                // 3. Niba nta service order → fallback to inspection plans
                try {
                    const plansResponse = await getInspectionPlansbycontract(node.ids);

                    return plansResponse.data.map((item) => ({
                        cancreateserviceorder: item.cancreateserviceorder || false,
                        id: item.inspectionplanid,
                        ids:node.ids,
                        name: `${item.expectedoutput} + Location (
         ${item.startfrom_location} to ${item.endat_location} )`,
                        type: 'inspectionplan',
                    }));
                } catch (error) {
                    toast.error('An error occurred while fetching inspection plan: ' + error);
                    return [];
                }
            } catch (error) {
             ...
             const response = await getInspectionPlansServiceorderbyserviceorder(node.id);
            ...
            return (
                response.data
                    ...
                    .map((item) => ({
                        ...
                        ids: node.id,
                        id: item.inspectionplanid,
                        name: `${item.expectedoutput} + Location (${item.startfrom_location} to ${item.endat_location} )`,
                        type: 'serviceorderinspectionplan',
    ...
    const InspectionPlans = () => {
        ...
        const[addmesg, setaddmsg] = useState(false);
        const fetchEvaluations = async () => {
            setLoading(true);
            try {
                //toast.info(`............${JSON.stringify(contractType)}`)
                const res = await getEvaluationsByPlan(contractType);
                if (res && res.data) {
                    
                    setEvaluations(res.data);
                    // toast.error("Error fetching evaluations"+JSON.stringify(res));
                } else setEvaluations([]);
            } catch (err) {
        ...
        const getPagedData = () => {
            ...
            if (searchQuery) {
                ...
                filtered = evaluations.filter((ev) => ev.roodname?.toLowerCase().includes(lower) || ev.comment?.toLowerCase().includes(lower) || ev.inspectiondate?.toLowerCase().includes(lower));
            }
        ...
        const  handleAddnew=async()=>{
                            ...
                            //getInstallmentsBycontractid addmesg, setaddmsg
                  
                const res = await getInstallmentsBycontractid(contractid);
                if (res ) { 
                    setSelectedEvaluation(paginatedEvaluations);
                    setShowAddModal(true);
                }else{
                   setaddmsg ("You cannot proceed with the evaluation as the payment settings have not yet been registered. please go on contract then payment setting and payment setting, dont forget to set installments") ;
        ...
        const handleApprovs = async (inspectionid, contractid, comment, status) => {
            if (!window.confirm('Are you sure you want to Approv this evaluation?')) return;
            try {
                await approvEvaluation(inspectionid, contractid, comment, status);
        ...
        const handleApproval = async (inspectionplanId) => {
            if (!window.confirm('Are you sure you want to delete this evaluation?')) return;
            try {
                await deleteEvaluation(inspectionplanId);
        ...
        const { totalCount, data: paginatedEvaluations } = getPagedData();

        return (
            <div className="container mt-4">
                <div className="d-flex justify-content-between align-items-center mb-3">
                    <h4>Inspection Evaluations<span style={{ color: "red" }}>{addmesg}</span></h4>
                    <Button
                        color="primary"
                        className="me-2"
                ...
                {/* Search */}
                <div className="mb-3">
                    <Input type="text" placeholder="Search by road name, comment, or date..." value={searchQuery} onChange={(e) => handleSearch(e.target.value)} />
                </div>

                {loading ? (
                    <p>Loading...</p>
                ) : totalCount === 0 ? (
                    <p className="text-center">No evaluations found</p>
                ) : (
                    <>
                ...
                <th>Delete</th> */}
                            ...
                            <tbody>
                                {paginatedEvaluations.map((ev, idx) => (
                                    <tr key={ev.inspectionID}>
                                        <td>{idx + 1 + (currentPage - 1) * pageSize}</td>
                                        <td>{ev.roodname}</td>
                                        <td>{formatDate(ev.inspectiondate)}</td>
                                        <td>{ev.comment}</td>

                                        {/** 
                  <td>{ev.proposedmax}</td>
                  <td>{ev.totalmax}</td>*/}
                                        <td>{ev.percentage}</td>
                                        <td>{ev.status}</td>
                                        {/** 
                  ...
                  <td>
                    <Button
                      ...
                      color="danger"
                      className="me-2"
                      onClick={() => handleDelete(ev.inspectionID)}
                    >
                      Delete
                    </Button></td>*/}
                                        <td>
                                       ...
                                       {ev.status!='Approved' && (<td>
                                            <Button size="sm" color="danger" className="me-2" onClick={() => handleApprovs(ev.inspectionid,ev.contractid,ev.comment,ev.status)}>
                                                Approv
                                            </Button>
                                        </td>)}
                                    </tr>
                ...
                {/* Modals */}
                <AddInspectionEvaluationModal
                    isOpen={showAddModal}
                    toggle={() => setShowAddModal(false)}
                    refresh={fetchEvaluations}
                    planId={contractType} // 👈 pass ID of selected plan
                />

                <UpdateInspectionEvaluationModal
                    isOpen={showUpdateModal}
                    toggle={() => setShowUpdateModal(false)}
                    evaluation={selectedEvaluation} // ← whole evaluation object
                    refresh={fetchEvaluations}
                    planId={contractType} // ← likely not correct
                />

                <EvaluationDetailModal isOpen={showDetailModal} toggle={() => setShowDetailModal(false)} evaluation={selectedEvaluation} refresh={fetchEvaluations} planId={contractType} />
            </div>
        );
    };
    ...
    const InspectionPlanServiceOrder = () => {
        ...
        const fetchEvaluations = async () => {
            ...
            try {
                //toast.info(`............${JSON.stringify(contractType)}`)
                const res = await getEvaluationsByPlanserviceorder(contractType);
                if (res && res.data) {
                    setEvaluations(res.data);
                    // toast.error("Error fetching evaluations"+JSON.stringify(res));
                } else setEvaluations([]);
        ...
        const  handleAddnew=async()=>{
                ...
                const res = await getInstallmentsByservice_order_id(serviceorderid);
                if ( res ) { 
                    setSelectedEvaluation(paginatedEvaluations);
                    setShowAddModal(true);
                }else{
        ...
        const handleDelete = async (inspectionplanId) => {
         ...
         const handleApprovs  = async (inspectionid, contractid, comment, status) => {
            if (!window.confirm('Are you sure you want to Approv this evaluation?')) return;
            try {
                //approvEvaluationserviceorder
                await approvEvaluationserviceorder(inspectionid, contractid, comment, status);
                toast.success('Evaluation Approved successfully');
                fetchEvaluations();
            } catch (err) {
                toast.error('Approval failed'+err);
        ...
        return (
            <div className="container mt-4">
                <div className="d-flex justify-content-between align-items-center mb-3">
                    <h4>Inspection Evaluationserv <span style={{ color: "red" }}>{addmesg}</span></h4>
                    <Button
                        color="primary"
                        className="me-2"
                        onClick={() => { handleAddnew()
                  ...
                  <td>
                    ...
                    >
                      Delete
                                        ...
                                        <td>
                                            <Button size="sm" color="danger" className="me-2" onClick={() => handleApprovs(ev.inspectionid,ev.serviceorderid,ev.comment,ev.status)}>
                                                Approv
                                            </Button>
                                        </td>
                                    </tr>
                ...
                {/* Modals   */}
                <AddInspectionEvaluationServiceOrderModal
                    isOpen={showAddModal}
                    toggle={() => setShowAddModal(false)}
                    refresh={fetchEvaluations}
                    planId={contractType} // 👈 pass ID of selected plan
                ...
                <UpdateInspectionEvaluationServiceOrderModal
                    isOpen={showUpdateModal}
                    toggle={() => setShowUpdateModal(false)}
                    evaluation={selectedEvaluation} // ← whole evaluation object
                    refresh={fetchEvaluations}
                ...
                <EvaluationDetailServiceOrderModal isOpen={showDetailModal} toggle={() => setShowDetailModal(false)} evaluation={selectedEvaluation} refresh={fetchEvaluations} planId={contractType} />
            </div>
        );
    };
            ...
            <div className="mailbox-layout flex max-w-[3200px] flex-1 overflow-hidden flex-col md:flex-row">
                ...
                <main className="content-scrollable bg-white max-w-[1200px] w-full mx-auto overflow-x-auto">
                    <div className="responsive-table">
                        {typename === 'inspectionplan' && (cancreateserviceorder ? '' : <InspectionPlans />)}
                        {typename === 'serviceorderinspectionplan' && <InspectionPlanServiceOrder />}
