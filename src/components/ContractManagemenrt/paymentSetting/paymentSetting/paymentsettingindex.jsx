...
import { ClipboardList, Menu, CreditCard, DollarSign, Banknote, Coins, Settings } from 'lucide-react';
//---------------------------------------
...
import { Modal, Button, ButtonGroup, Dropdown, OverlayTrigger, Tooltip } from 'react-bootstrap';
...
//-----------------------------------------
//import AddInspectionPlanserviceorderModal from './AddPaymentSettingModalserviceorderModal';
//import UpdateInspectionPlanserviceorderModal from './UpdatePaymentSettingServiceOrderModal';
...
import {
    ...
    getAllPaymentSettingbycontractid,
    deletePaymentSetting,
    savePaymentSetting,
    getInstallmentsBySettingId,
    getAllPaymentSettingbyserviceorder,
} from '../../../../services/ContractManagement/ContractSetting/contractpaymentService';

import AddPaymentSettingModal from './AddPaymentSettingModal';
import AddPaymentSettingModalserviceorderModal from './AddPaymentSettingModalserviceorderModal'
import UpdatePaymentSettingServiceOrderModal from './UpdatePaymentSettingServiceOrderModal'
import UpdatePaymentSettingModal from './UpdatePaymentSettingModal';
...
const Mailbox = () => {
    ...
    const PaymentSettings = () => {
        ...
        const fetchSettings = async () => {
            setLoading(true);
            try {
                const res = await getAllPaymentSettingbycontractid(contractType);

                //const res = await getAllPaymentSettingbycontractid(contractType);
                //const res = await getAllPaymentSettings();
                if (res) {
                    setSettings(res);
                } else {
                    setSettings([]);
        ...
        const getPagedData = () => {
            ...
            if (searchQuery) {
                ...
                filtered = settings.filter(
                    (s) =>
                        s.source_type?.toLowerCase().includes(lower) ||
                        s.tax_type?.toLowerCase().includes(lower) ||
                        String(s.contract_id)?.includes(lower) ||
                        String(s.service_order_id)?.includes(lower)
                );
            }
        ...
        const { totalCount, data: paginatedSettings } = getPagedData();
        ...
        return (
            <div className="container mt-4">
                ...
                {/* 🔍 Search Box */}
                <div className="mb-3">
                    <Input type="text" placeholder="Search by contract, service, type..." value={searchQuery} onChange={(e) => handleSearch(e.target.value)} />
                </div>

                {loading ? (
                    <p>Loading...</p>
                ) : totalCount === 0 ? (
                    <p className="text-center">No payment settings found</p>
                ) : (
                    <>
                        <Table bordered hover className="custom-thead">
                            ...
                            <tbody>
                                {paginatedSettings.map((s, idx) => (
                                    <tr key={s.id}>
                                        <td>{idx + 1 + (currentPage - 1) * pageSize}</td>
                                        <td>
                                            {s.contract_refnumber}
                                            {'-'}
                                            {s.contract_discription}
                                        </td>
                                        <td>{s.service_order_id}</td>
                                        <td>{s.source_type}</td>
                                        <td>{s.tax_type}</td>
                                        <td>{s.advance_payment_percent}</td>
                                        <td>{s.number_of_installments}</td>
                                        <td className="text-center">
                                            ...
                                            <Dropdown.Menu>
                                                ...
                                                >
                                                    <FaList className="text-secondary me-2" />
                                                    Installments
                                                </Dropdown.Item>

                                                <Dropdown.Item onClick={() => handleDelete(s.id)}>
                ...
                {/* Modals */}
                <AddPaymentSettingModal isOpen={showAddModal} toggle={() => setShowAddModal(false)} refresh={fetchSettings} contractType={contractType} />
                <UpdatePaymentSettingModal isOpen={showUpdateModal} toggle={() => setShowUpdateModal(false)} refresh={fetchSettings} selectedSetting={selectedSetting || {}}  />
                <PaymentInstallmentsModal isOpen={showInstallmentsModal} toggle={() => setShowInstallmentsModal(false)} setting={selectedSetting || {}} />
            </div>
        );
    };
    ...
    const InspectionPlanServiceOrder = () => {
        ...
        const fetchSettings = async () => {
            setLoading(true);
            try {
                const res = await getAllPaymentSettingbyserviceorder(contractType);
        ...
        return (
            <div className="container mt-4">
                            ...
                            <tbody>
                                {paginatedSettings.map((s, idx) => (
                                    <tr key={s.id}>
                                        ...
                                        <td>{s.serviceorder_description}</td>
                                        <td>{s.source_type}</td>
                                        <td>{s.tax_type}</td>
                                        <td>{s.advance_payment_percent}</td>
                                        <td>{s.number_of_installments}</td>
                ...
                {/* Modals   */}
                <AddPaymentSettingModalserviceorderModal isOpen={showAddModal} toggle={() => setShowAddModal(false)} refresh={fetchSettings} contractType={contractType} />
                <UpdatePaymentSettingServiceOrderModal isOpen={showUpdateModal} toggle={() => setShowUpdateModal(false)} refresh={fetchSettings} selectedSetting={selectedSetting || {}}  />
                <PaymentInstallmentsModal isOpen={showInstallmentsModal} toggle={() => setShowInstallmentsModal(false)} setting={selectedSetting || {}} />
            </div>
        );
    };
            ...
            <div className="mailbox-layout flex max-w-[3200px] flex-1 overflow-hidden flex-col md:flex-row">
                    ...
                    <div className="sidebar-header flex items-center justify-between p-4 border-b border-gray-200 bg-gradient-to-r from-blue-50 to-blue-100">
                        <h2 className="text-lg font-semibold text-gray-700 flex items-center gap-2">
                            <span className="relative flex items-center justify-center">
                                <Settings className="w-5 h-5 text-black-1700" />
                                <Coins className="absolute bottom-2 w-6 h-6 text-red-600" />
                            </span>
                            Payment setting
                        </h2>
                ...
                <main className="content-scrollable bg-white max-w-[1200px] w-full mx-auto overflow-x-auto">
                    <div className="responsive-table">
                        {typename === 'serviceorder' && (cancreateserviceorder ? '' : <PaymentSettings />)}
