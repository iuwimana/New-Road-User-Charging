    ...
    const handleDownload= async (inspectionplanid,contractid, startfrom_location,endat_location,expectedoutput,inspectiondate,status) => {
        ...
        return (
            <div className="container mt-4">
                                        ...
                                        {plan.status === 'planned' && (
                                            ...
                                            {plan.status === 'planned with Activities' && (
                                                ...
                                                >
                                                    ...
                                                    <Button size="sm" color="success" className="ms-2" onClick={() =>handleApprovals(plan.inspectionplanid,plan.contractid, plan.startfrom_location,plan.endat_location,plan.expectedoutput,plan.inspectiondate,plan.status)}>
                                                        Approve
                                                    </Button>
                                                    <Button
                                                        size="sm"
                                            ...
                                            {plan.status === 'Approved' && (
                                                 <>
                                                    ...
                                                    <Button size="sm" color="success" className="ms-2" onClick={() =>handleDownload(plan.inspectionplanid,plan.contractid, plan.startfrom_location,plan.endat_location,plan.expectedoutput,plan.inspectiondate,plan.status)}>
                ...
                <InspectionPlanRejectedModal isOpen={showRejectModal} toggle={() => setShowRejectModal(false)} plan={selectedPlan || {}} refresh={fetchPlans}  />
            </div>
        );
    };
                    ...
                    <Card className="shadow border-0">
                        <CardHeader className="card-header bg-transparent">
                            <div className="text-center">
                                <h1>Contract Settings - Road Contractok</h1>
                            </div>
                        </CardHeader>
                        <CardBody>
