...
import { GiGasPump } from "react-icons/gi"; // Fuel pump icon
import { useEffect, useState } from 'react';
import { FaGasPump } from "react-icons/fa"; // Alternative fuel icon
import { toast } from 'react-toastify';
// or
import { IoMdCash } from "react-icons/io"; // Cash icon for revenue
import { MdTraffic } from "react-icons/md"; // Traffic flow
...
const Analytics = () => {
    ...
    useEffect(() => {
        const fetchData = async () => {
            try {
                const fiscalyearsid = 0; // Or get this from props/state if needed
                
                // Fetch both data sources
                const sourcesPromise = Revcor.getrevenucorrectionByFiscalYearID(15);
                const revenuePromise = AnnualTargets.getannualtargetByFiscalyears(15);

                // Handle responses differently based on their structure
                const sourcesResponse = await sourcesPromise;
                const revenueResponse = await revenuePromise;

                // Extract data safely - handle both AxiosResponse and direct data returns
                const sources = (sourcesResponse as AxiosResponse)?.data ?? sourcesResponse;
                const revenue = (revenueResponse as AxiosResponse)?.data ?? revenueResponse;

                if (!Array.isArray(sources) || !Array.isArray(revenue)) {
                    throw new Error('Invalid data format received from the server');
                }

                // Calculate totals safely
                const deposits = sources.map(source => 
                    source.deposit || source.totalamount || 0
                );
                const totalDeposit = deposits.reduce((acc, curr) => acc + curr, 0);
                const totalTarget = revenue[0]?.sum || 0;
                const balance = totalDeposit - totalTarget;
    ...
    useEffect(() => {
        const populateFiscalYears = async () => {
            try {
                const response = await FiscalYear.getFiscalyears();
                if (response.data && response.data.length > 0) {
                    setFiscalYears(response.data);
                    // Set the first fiscal year as default
                    setSelectedFiscalYearId(response.data[0].fiscalyearid);
                } else {
                    toast.error('No fiscal years found');
                }
            } catch (ex) {
                toast.error('Error loading fiscal years: ' + ex);
            }
        };
    ...
    // Fetch financial data when fiscal year changes
    useEffect(() => {
        const fetchFinancialData = async () => {
            ...
            try {
                ...
                const [sourcesRes, revenueRes] = await Promise.all([
                    Revcor.getrevenucorrectionByFiscalYearID(selectedFiscalYearId),
                    AnnualTargets.getannualtargetByFiscalyears(selectedFiscalYearId)
                ]);

                // Handle different response structures
                const sources = sourcesRes.data?.data || sourcesRes.data || sourcesRes;
                const revenue = revenueRes.data?.data || revenueRes.data || revenueRes;
                ...
                // Calculate totals
                const deposits = sources.map(source => 
                    Number(source.deposit) || 
                    Number(source.totalamount) || 
                    0
                );
                
                const totalDeposit = deposits.reduce((acc, curr) => acc + curr, 0);
                const totalTarget = Number(revenue[0]?.sum) || 0;
                const balance = totalDeposit - totalTarget;
        ...
        fetchFinancialData();
    }, [selectedFiscalYearId]); // Re-run when fiscal year changes
    ...
    if (financialData.error) {
        return (
            <div className="panel h-full flex items-center justify-center text-red-500">
        ...
        <div>
            ...
            <div className="pt-5">
                <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-6 mb-6">
                        ...
                        <div className="grid sm:grid-cols-2 gap-8 text-sm text-[#515365] font-bold">
                            <div>
                                <div>
                                    ...
                                    <div className="text-[#f8538d] text-lg">{financialData.loading ? 'Loading...' : financialData.totaldeposit.toLocaleString()}</div>
                                </div>
                            ...
                            <div>
                                <div>
                                    ...
                                    <div className="text-[#f8538d] text-lg">{financialData.loading ? 'Loading...' : financialData.totalTarget.toLocaleString()}</div>
                                </div>
                    ...
                    >
                        <div className="flex items-start justify-between text-white-light mb-1 z-[7]">
                        
                                        
                                            
                                       

                            <h5 className="font-semibold text-lg"><FaBalanceScale size={30} color="#28a745" />Balance</h5>

                            <div className="relative text-xl whitespace-nowrap">
                            {financialData.loading ? 'Loading...' : financialData.balance.toLocaleString()}
                                <span className="table text-[#d3d3d3] bg-[#4361ee] rounded p-1 text-xs mt-1 ltr:ml-auto rtl:mr-auto">Rfw</span>
                            </div>
                            
                        </div>
                        <div className="flex items-start justify-between text-white-light mb-1 z-[7]">
                            <h5 className="font-semibold text-lg"><FcCurrencyExchange size={30} color="#007bff" />Target</h5>

                            <div className="relative text-xl whitespace-nowrap">
                            {financialData.loading ? 'Loading...' : financialData.totalTarget.toLocaleString()}
                                <span className="table text-[#d3d3d3] bg-[#4361ee] rounded p-1 text-xs mt-1 ltr:ml-auto rtl:mr-auto">Rfw</span>
                            </div>
                            
                        </div>
                        <div className="flex items-start justify-between text-white-light mb-1 z-[7]">
                            <h5 className="font-semibold text-lg"><FcCheckmark size={40} />Revenu</h5>

                            <div className="relative text-xl whitespace-nowrap">
                            {financialData.loading ? 'Loading...' : financialData.totaldeposit.toLocaleString()}
                                <span className="table text-[#d3d3d3] bg-[#4361ee] rounded p-1 text-xs mt-1 ltr:ml-auto rtl:mr-auto">Rfw</span>
                            </div>
                ...
                <div className="grid lg:grid-cols-3 gap-6 mb-6">
                    <div className="panel h-full p-0 lg:col-span-2">
                        <div className="flex items-start justify-between dark:text-white-light mb-5 p-5 border-b  border-white-light dark:border-[#1b2e4b]">
                            <h5 className="font-semibold text-lg ">Revenue collected per months-2024/2025</h5>
                            <div className="dropdown">
                                <Dropdown
                                    offset={[0, 5]}
                                    placement={`${isRtl ? 'bottom-start' : 'bottom-end'}`}
                    ...
                    <div className="panel h-full">
                        <div className="flex items-start justify-between dark:text-white-light mb-5 -mx-5 p-5 pt-0 border-b  border-white-light dark:border-[#1b2e4b]">
                            <h5 className="font-semibold text-lg ">All Revenue Resources</h5>
                            <div className="dropdown">
                                <Dropdown
                                    offset={[0, 5]}
                                    placement={`${isRtl ? 'bottom-start' : 'bottom-end'}`}
                            ...
                            <div className="space-y-7">
                                <div className="flex">
                                    ...
                                    <div>
                                        ...
                                        <p className="text-white-dark text-xs">2024/2025</p>
                                    </div>
                                </div>
                                <div className="flex">
                                    <div className="shrink-0 ltr:mr-2 rtl:ml-2 relative z-10 before:w-[2px] before:h-[calc(100%-24px)] before:bg-white-dark/30 before:absolute before:top-10 before:left-4">
