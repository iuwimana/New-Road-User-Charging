...
import IconCaretsDown from '../Icon/IconCaretDown';
...
const Header = () => {
    ...
    const [textSize, setTextSize] = useState('medium-text');
    const [textSize1, setTextSize1] = useState('small-text');
    //------------------------------------------
    useEffect(() => {
        const fetchProgram = async () => {
            try {
                const response: AxiosResponse<any> | null = await FiscalYear.getFiscalyears();
                if (response) {
                    const fiscalYears: FiscalYearData[] = response.data;
                    setData(response);
                    const fiscalyearid: number[] = fiscalYears.map((year) => year.fiscalyearid);
                    const fiscalyear: String[] = fiscalYears.map((year) => year.fiscalyear);
            ...
            <div className="container mt-3 ">
                <div className="shadow-sm text-end ">
                    <div className="horizontal-logo flex lg:hidden justify-between items-center ltr:mr-2 rtl:ml-2">
                        <Link to="/analytics" className="main-logo flex items-center shrink-0">
                            <img className="w-8 ml-[5px] flex-none" src="/assets/images/rwanda.png" alt="logo" />
                            <span className="text-2xl ltr:ml-1.5 rtl:mr-1.5  font-semibold  align-middle hidden md:inline dark:text-white-light transition-all duration-300">RUCS</span>
                        </Link>
                        <button
                            type="button"
                    ...
                    <div className="relative bg-white flex w-full items-end px-5 py-2.5 dark:bg-black">
                        ...
                        <div className="sm:flex-1 ltr:sm:ml-0 ltr:ml-auto sm:rtl:mr-0 rtl:mr-auto flex items-end space-x-1.5 lg:space-x-2 rtl:space-x-reverse dark:text-[#d0d2d6]">
